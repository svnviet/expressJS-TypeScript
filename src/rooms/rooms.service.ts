...
export const getRoomsByStatusAndFloorId = async (status: Room['status'], floor_id: Room['floor_id']) => {
    ...
    WHERE rooms.status = ? AND rooms.floor_id = ? AND rooms.deleted_at IS NULL
...
/**
 * soft delete room status by id
 */
export const deleteRoomStatusById = async (id: Room['id']) => {
    const result = await roomRepository.update(id, { status: 9, updated_at: new Date() });
    return result.affected > 0;
};